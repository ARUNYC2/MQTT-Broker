...
public abstract class WireMessage {
	...
	//The MQTT message ID
	protected int msgId;
	public boolean isCon=false;//.socket read loop close if client send disconnect packet 
...
*/
	public  WireMessage createWireMessage(InputStream inputStream,Socket socket) //throws MqttException 
        {     
            //System.out.println("WireMessage ,createWireMessage");
            
             if(flag==0)
             {
                 Thread t=new Thread (new SocketWrite(qs,socket));
                        t.start();
                flag=1;
             }
		try {
			...
			if (type == WireMessage.MESSAGE_TYPE_CONNECT) {
				result = new Connect(info, data,qs);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PUBLISH) {
				result = new Publish(info, data);
                            try {
                                qs.SubmitInQueue(result);
                            } catch (InterruptedException ex) {
                                Logger.getLogger(WireMessage.class.getName()).log(Level.SEVERE, null, ex);
                            }
                           }
			else if (type == WireMessage.MESSAGE_TYPE_PUBACK) {
				result = new PubAck(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PUBCOMP) {
				result = new PubComp(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_CONNACK) {
				result = new Connack(info, data,qs);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PINGREQ) {
				result = new PingReq(info, data,qs);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PINGRESP) {
				result = new PingResp(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_SUBSCRIBE) {
				result = new Subscribe(info, data,socket,qs);
			}
			else if (type == WireMessage.MESSAGE_TYPE_SUBACK) {
				result = new Suback(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_UNSUBSCRIBE) {
				result = new Unsubscribe(info, data,socket,qs);
			}
			else if (type == WireMessage.MESSAGE_TYPE_UNSUBACK) {
				result = new UnsubAck(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PUBREL) {
				result = new PubRel(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_PUBREC) {
				result = new PubRec(info, data);
			}
			else if (type == WireMessage.MESSAGE_TYPE_DISCONNECT) {
				result = new Disconnect(info, data,socket);
			}
		
                        else {
				//throw ExceptionHelper.createMqttException(MqttException.REASON_CODE_UNEXPECTED_ERROR);
			new MqttException("Packet not Match");
                        }
			return result;
		} catch(IOException io) {
			//throw new MqttException(io);
                        new MqttException("IOException in WireMessage,createWire");
                        return null;
		}
	}
		
	protected  byte[] encodeMBI( long number) {
		int numBytes = 0;
		long no = number;
	 ...
	 */
	protected  MultiByteInteger readMBI(DataInputStream in)throws IOException {
	...
	/*	catch(UnsupportedEncodingException ex)
		...
		throw new MqttException(ex);}
          */		 catch (IOException ex) {
		      new MqttException(ex+" encodeUTF8 in wireMessage,createWire ");
		}
	}
	...
	protected String decodeUTF8(DataInputStream input) //throws MqttException
	{
		...
		try {
			...
			//throw new MqttException(ex);
                        System.out.println("IOException..decodeUTF8 ..wire_message");
                        return null;
		}
	}
